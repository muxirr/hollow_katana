...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  try {
    ...
    MessageBox(hwnd, errMsg, _T("Resource loading failure"),
               MB_OK | MB_ICONERROR);
    return -1;
  }
  ...
  while (!isQuit) {
    ...
    nanoseconds sleepDuration =
        duration_cast<nanoseconds>(steady_clock::now() - frameStart);
...
static void drawBackground() {
  static IMAGE *imgBackground =
      ResourcesManager::Instance()->findImage(_T("background"));
  static Rect rectDst = {(getwidth() - imgBackground->getwidth()) / 2,
...
void readConfig(HWND hwnd) {
  std::ifstream ifs("./assets/config.ini");
  if (!ifs.is_open()) {
    MessageBox(hwnd, _T("Unable to open assets/config.ini"), _T("Error"),
               MB_OK | MB_ICONERROR);
    std::exit(-1);
  }
...
static void drawRemianHp() {
  static IMAGE *imgUiHeart =
      ResourcesManager::Instance()->findImage(_T("uiHeart"));
  static IMAGE *enemyIdle =
      ResourcesManager::Instance()->findAtlas(_T("enemyIdleLeft"))->getImage(0);
  static IMAGE *playerIdle =
      ResourcesManager::Instance()->findImage(_T("playerIdleRight"));

  static int w = playerIdle->getwidth() / 5;
  static float playerScale = playerIdle->getheight() / static_cast<float>(w);
  static float enemyScale =
      static_cast<float>(enemyIdle->getheight()) / enemyIdle->getwidth();

  static Rect rectDst = {0, 10, imgUiHeart->getwidth(),
                         imgUiHeart->getheight()};
  static Rect rectPlayerDst = {
      0, 5, static_cast<int>(imgUiHeart->getheight() / playerScale),
      imgUiHeart->getheight()};
  static Rect rectEnemyDst = {
      1240, 10, static_cast<int>(imgUiHeart->getheight() / enemyScale),
      imgUiHeart->getheight()};
  static Rect rectPlayerSrc = {0, 0, w, playerIdle->getheight()};
  static Rect rectEnemySrc = {0, 0, enemyIdle->getwidth(),
                              enemyIdle->getheight()};
